...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        action_1__order = When(action_expired__condition, then=ACTION_EXPIRED)

        action_1__price = When(action_1__condition, then=('product__price') * F('quantity') * action_1__discount)
        action_2__price = When(action_2__condition, then=('product__price') * F('quantity') * action_2__discount)
        action_expired__price = When(action_expired__condition,
                                     then=('product__price') * F('quantity') * action_expired__discount)
        ...
        for orderitem in test_orders:
            print(f'{orderitem.action_order:2}: заказ №{orderitem.pk:3}'
                  f'{orderitem.product.name:15}: скидка {abs(orderitem.total_price):12.2f}.руб |'
